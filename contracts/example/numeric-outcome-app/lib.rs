...
mod numeirc_outcome_app {
    ...
    mod tests {
        ...
        fn update_by_state_and_finalized() {
            let mut numeric_outcome_app = NumericOutcomeApp::new();
            assert_eq!(numeric_outcome_app.update_by_state(1), 3);
            assert_eq!(numeric_outcome_app.get_app_state(), 1);
        }

        #[test]
        fn update_by_action_and_finalized() {
            let mut numeric_outcome_app = NumericOutcomeApp::new();
            assert_eq!(numeric_outcome_app.update_by_action(1), 3);
            assert_eq!(numeric_outcome_app.get_app_state(), 1);
        }
        ...
        fn get_outcome() {
            let mut numeric_outcome_app = NumericOutcomeApp::new();
            assert_eq!(numeric_outcome_app.update_by_action(2), 3);
            assert_eq!(numeric_outcome_app.get_outcome(10), 20);
        }
    }
}
