...
mod numeric_outcome_app {
    ...
    mod tests {
        ...
        fn initiate_works() {
            ...
            NumericOutcomeApp::new(initiate_request);
        }
        ...
        fn update_by_action_fail() {
            let (_, accounts) = accounts_setup();
            let initiate_request = get_initiate_request(0, accounts, 2);
            let mut numeric_outcome_app = NumericOutcomeApp::new(initiate_request);
            // app not in action mode
            numeric_outcome_app.update_by_action(1);
        }
        ...
        fn update_by_state_works() {
            ...
            numeric_outcome_app.update_by_state(state_proof.0, state_proof.1);
            assert_eq!(numeric_outcome_app.get_outcome(5), 25);
            assert_eq!(numeric_outcome_app.is_finalized(), false);
        }
        ...
        fn invalid_sigs_fail() {
            ...
            numeric_outcome_app.update_by_state(app_state, invalid_sigs);
        }
        ...
        fn update_by_action_works() {
            ...
            let state_proof = get_state_proof(5, 0, 2, 2, players_pair);
            numeric_outcome_app.update_by_state(state_proof.0, state_proof.1);
            let settle_finalized_time = numeric_outcome_app.get_settle_finalized_time().unwrap();
            advance_blocks(settle_finalized_time + 1);
            numeric_outcome_app.update_by_action(1);
            assert_eq!(numeric_outcome_app.get_outcome(1), 1);
        }
        ...
        fn update_by_state_and_finalized() {
            ...
            numeric_outcome_app.update_by_state(state_proof.0, state_proof.1);
            assert_eq!(numeric_outcome_app.get_outcome(2), 4);
            assert_eq!(numeric_outcome_app.is_finalized(), true);
        }
        ...
        fn update_by_action_after_finalized_fail() {
            ...
            let state_proof = get_state_proof(2, 0, 2, 2, players_pair);
            numeric_outcome_app.update_by_state(state_proof.0, state_proof.1);
            numeric_outcome_app.update_by_action(2);
        }
        ...
        fn finalize_on_timeout_works() {
            ...
            numeric_outcome_app.update_by_state(state_proof.0, state_proof.1);
            advance_blocks(5);
            numeric_outcome_app.finalize_on_timeout();
            assert_eq!(numeric_outcome_app.is_finalized(), true);
        }
    }
}
