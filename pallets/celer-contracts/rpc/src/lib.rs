...
use celer_contracts_primitives::RentProjection;
...
pub use celer_contracts_rpc_runtime_api::{
...
struct ContractAccessError(celer_contracts_primitives::ContractAccessError);
impl From<ContractAccessError> for Error {
	fn from(e: ContractAccessError) -> Error {
		use celer_contracts_primitives::ContractAccessError::*;
		match e.0 {
			DoesntExist => Error {
...
pub trait ContractsApi<BlockHash, BlockNumber, AccountId, Balance, Hash> {
	...
	/// Returns the on-chain address corresponding to off-chain address.
	#[rpc(name = "contracts_resolve")]
	fn resolve(
		&self,
		virt_addr: Hash,
		at: Option<BlockHash>
